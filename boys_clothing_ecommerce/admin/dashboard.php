...
try {
    // Total sales count
    $stmt = $pdo->prepare("SELECT COUNT(*) as total_sales, SUM(p.price) as total_sales_amount, SUM(p.price*0.15) as total_commission FROM orders o JOIN products p ON o.product_id = p.id WHERE o.status='delivered'");
    $stmt->execute();
    $summary = $stmt->fetch(PDO::FETCH_ASSOC);
    $totalSalesCount = $summary['total_sales'] ?? 0;
    $totalSalesAmount = $summary['total_sales_amount'] ?? 0;
    $totalCommission = $summary['total_commission'] ?? 0;

    // Total payouts
    $stmt = $pdo->prepare("SELECT SUM(amount) as total_payouts FROM payouts");
    $stmt->execute();
    $totalPayouts = $stmt->fetch(PDO::FETCH_ASSOC)['total_payouts'] ?? 0;
} catch(PDOException $e){
    $totalSalesCount = $totalSalesAmount = $totalCommission = $totalPayouts = 0;
}
    ...
    <div class="row text-center mb-4">
        <div class="col-md-3 mb-2">
            <div class="card bg-light shadow-sm p-3">
                <h5>Total Sales</h5>
                <p class="fs-4"><?php echo $totalSalesCount; ?></p>
            </div>
        </div>
        <div class="col-md-3 mb-2">
            <div class="card bg-light shadow-sm p-3">
                <h5>Sales Amount ($)</h5>
                <p class="fs-4"><?php echo number_format($totalSalesAmount,2); ?></p>
            </div>
        </div>
        <div class="col-md-3 mb-2">
            <div class="card bg-light shadow-sm p-3">
                <h5>Total Payouts ($)</h5>
                <p class="fs-4"><?php echo number_format($totalPayouts,2); ?></p>
            </div>
        </div>
        <div class="col-md-3 mb-2">
            <div class="card bg-light shadow-sm p-3">
                <h5>Commission Earned ($)</h5>
                <p class="fs-4"><?php echo number_format($totalCommission,2); ?></p>
            </div>
        </div>
    </div>
                                    ...
                                    <td>
                                        <?php if($r['status']=='pending'): ?>
                                            <form method="POST" action="payouts.php" class="d-flex gap-1">
                                                ...
                                                <input type="text" name="transaction_number" class="form-control form-control-sm" placeholder="Transaction #">
